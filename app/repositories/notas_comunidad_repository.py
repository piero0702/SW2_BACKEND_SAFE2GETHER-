...
class NotasComunidadRepository:
    def __init__(self, client: SupabaseClient | None = None):
        self.client = client or SupabaseClient()

    def _url(self):
        return table_url('Notas_Comunidad')

    async def list_notas(self) -> List[Dict[str, Any]]:
    ...
    async def get_by_id(self, nota_id: int) -> Dict[str, Any] | None:
        params = {"select": "*", "id": f"eq.{nota_id}", "limit": 1}
        res = await self.client.get(self._url(), params=params)
        try:
            res.raise_for_status()
    ...
    async def create_nota(self, payload: dict) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                body = exc.response.text
            logger.error(
                "Supabase create_nota failed: status=%s url=%s payload=%s response_body=%s",
    ...
    async def update_nota(self, nota_id: int, payload: dict) -> Dict[str, Any]:
        params = {"id": f"eq.{nota_id}", "select": "*"}
        ...
        try:
            ...
            try:
                ...
                body = exc.response.text
            logger.error(
                "Supabase update_nota failed: status=%s url=%s payload=%s response_body=%s",
                exc.response.status_code, exc.request.url, payload, body
            )
            detail = body if isinstance(body, (dict, list, str)) else str(body)
            raise HTTPException(status_code=exc.response.status_code, detail=detail)
    ...
    async def delete_nota(self, nota_id: int) -> int:
        ...
        try:
            ...
            try:
                ...
                body = exc.response.text
            logger.error(
                "Supabase delete_nota failed: status=%s url=%s params=%s response_body=%s",
                exc.response.status_code, exc.request.url, params, body
            )
            detail = body if isinstance(body, (dict, list, str)) else str(body)
            raise HTTPException(status_code=exc.response.status_code, detail=detail)
