...
class ComentariosRepository:
    ...
    def _url(self):
        # La tabla en la imagen/DB se llama "Comentarios"
        return table_url('Comentarios')

    async def list_comentarios(self) -> List[Dict[str, Any]]:
    ...
    async def get_by_id(self, comentario_id: int) -> Dict[str, Any] | None:
        params = {"select": "*", "id": f"eq.{comentario_id}", "limit": 1}
        res = await self.client.get(self._url(), params=params)
        try:
            res.raise_for_status()
    ...
    async def create_comentario(self, payload: dict) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                body = exc.response.text
            logger.error(
                "Supabase create_comentario failed: status=%s url=%s payload=%s response_body=%s",
    ...
    async def update_comentario(self, comentario_id: int, payload: dict) -> Dict[str, Any]:
        params = {"id": f"eq.{comentario_id}", "select": "*"}
        ...
        try:
            ...
            try:
                ...
                body = exc.response.text
            logger.error(
                "Supabase update_comentario failed: status=%s url=%s payload=%s response_body=%s",
                exc.response.status_code, exc.request.url, payload, body
            )
            detail = body if isinstance(body, (dict, list, str)) else str(body)
            raise HTTPException(status_code=exc.response.status_code, detail=detail)
    ...
    async def delete_comentario(self, comentario_id: int) -> int:
        params = {"id": f"eq.{comentario_id}"}
        ...
        try:
            ...
            try:
                ...
                body = exc.response.text
            logger.error(
                "Supabase delete_comentario failed: status=%s url=%s params=%s response_body=%s",
                exc.response.status_code, exc.request.url, params, body
            )
            detail = body if isinstance(body, (dict, list, str)) else str(body)
            raise HTTPException(status_code=exc.response.status_code, detail=detail)
