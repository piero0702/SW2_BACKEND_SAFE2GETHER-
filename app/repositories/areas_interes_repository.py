...
class AreasInteresRepository:
    ...
    async def list_active_areas(self) -> List[Dict[str, Any]]:
        """Obtiene todas las áreas activas para procesamiento de notificaciones"""
        params = {"select": "*", "activo": "eq.true"}
        res = await self.client.get(self._url(), params=params)
        res.raise_for_status()
        return res.json()

    async def get_by_id(self, area_id: int) -> Dict[str, Any] | None:
        params = {"select": "*", "id": f"eq.{area_id}", "limit": 1}
        res = await self.client.get(self._url(), params=params)
        try:
            res.raise_for_status()
    ...
    async def create_area(self, payload: dict) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                body = exc.response.text
            logger.error(
                "Supabase create_area failed: status=%s url=%s payload=%s response_body=%s",
    ...
    async def update_area(self, area_id: int, payload: dict) -> Dict[str, Any]:
        params = {"id": f"eq.{area_id}", "select": "*"}
        ...
        try:
            ...
            try:
                ...
                body = exc.response.text
            logger.error(
                "Supabase update_area failed: status=%s url=%s payload=%s response_body=%s",
                exc.response.status_code, exc.request.url, payload, body
            )
            detail = body if isinstance(body, (dict, list, str)) else str(body)
            raise HTTPException(status_code=exc.response.status_code, detail=detail)
    ...
    async def delete_area(self, area_id: int) -> int:
        ...
        try:
            ...
            try:
                ...
                body = exc.response.text
            logger.error(
                "Supabase delete_area failed: status=%s url=%s params=%s response_body=%s",
                exc.response.status_code, exc.request.url, params, body
            )
            detail = body if isinstance(body, (dict, list, str)) else str(body)
            raise HTTPException(status_code=exc.response.status_code, detail=detail)
